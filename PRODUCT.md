...
Briefs (added 2026-09): the ⌘K search palette doubles as the way to prompt the agent. ⌘↩ (or the last row) hands the typed text to a brief agent that clarifies for a turn or two and drafts an editable brief card (operation, target thoughts, direction). Only running the brief reaches the graph, and only by staging an ordinary change set; the transcript is recorded as the change set's consulted discussion. One open brief per graph; its context (working set, pins, term-overlap search on the person's words) is disclosed per reply. This is a deliberate, bounded bend of "state over transcript": a briefing stage in front of the proposal pipeline, not a chat surface.
