...
Relations are typed, directional edges. Choose the type by what the edge asserts: supports — the source gives a reason or evidence to believe the target; contradicts — the source and target cannot both hold as stated; depends_on — the source loses force if the target fails (a premise, assumption, or prerequisite); example_of — the source is a concrete instance of the target's general idea; supersedes — the source replaces the target as the current formulation. Do not default to supports: an argument's structure is usually a mix of premises (depends_on), instances (example_of), and tensions (contradicts), and an edge mislabeled supports hides that structure. related_to is an explicit escape hatch for connections that matter but fit no specific type. When you notice a contradiction between thoughts — including ones written long apart — flag it with a contradicts relation rather than smoothing it over.
...
A testable expectation about the future becomes a prediction with explicit confidence; a concrete observation or cited fact becomes evidence linked (supports/contradicts) to the claims it bears on. The source's argumentative structure lives in the relations: which claims rest on which premises (depends_on), which observations bear on which claims (supports/contradicts), which passages instantiate which general ideas (example_of). A set of accurate but disconnected thoughts fails to preserve the argument, so budget operations for relations from the start — as a guide, roughly one relation for every two thoughts — and if the cap forces a trade, drop a peripheral thought before dropping the edges that carry the argument. Before returning, check each substantive section for omitted ideas, each thought for merged claims that could be challenged independently, and each thought for missing edges — a thought related to nothing is usually a missed dependency, instance, or tension, not a genuinely isolated idea.
