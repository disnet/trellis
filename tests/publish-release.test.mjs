...
// The scene: two thoughts, a relation, a pin, one group.
...
db.prepare("INSERT INTO relations (id, from_thought_id, to_thought_id, type, created_by, graph_id, created_at) VALUES ('r-pub1', ?, ?, 'supports', 'human', ?, ?)").run(b, a, graphId, Date.now());
...
const config = { workingSetId: ws, treatmentId: null, title: 'Test Garden', summary: 'An intro in my own words.' };

test('the first release publishes the selection and nothing else', async () => {
	const plan = release.buildReleasePlan(graphId, config, did);
	assertBoundary(plan);
	assert.deepEqual(
		plan.puts.map((p) => p.kind).sort(),
		['garden', 'relation', 'revision', 'revision', 'thought', 'thought']
	);
	assert.equal(plan.deletes.length, 0);
	assert.equal(plan.newThoughts.length, 2);
	...
	assert.equal(garden.release.thoughts.length, 2);
	assert.equal(garden.release.revisions.length, 2);
	assert.equal(garden.release.relations.length, 1);
	...
	assert.equal(outcome.results.filter((r) => r.ok).length, 6);
	// The manifest advances last, so readers never see a half release.
	const putCalls = calls.filter((c) => c.url.includes('putRecord'));
	assert.equal(putCalls.at(-1).body.collection, lex.COLLECTIONS.garden);
	assert.equal(db.prepare('SELECT COUNT(*) AS n FROM published_records WHERE graph_id = ?').get(graphId).n, 6);
});
...
test('a local edit becomes one public revision with prev, authorship, and the change note', async () => {
	...
	assert.equal(db.prepare("SELECT COUNT(*) AS n FROM published_records WHERE graph_id = ? AND collection = ?").get(graphId, lex.COLLECTIONS.revision).n, 3);
});

test('leaving the selection withdraws the thought, its history, and its relations', async () => {
	db.prepare('DELETE FROM working_set_items WHERE working_set_id = ? AND thought_id = ?').run(ws, b);
	const plan = release.buildReleasePlan(graphId, config, did);
	assert.deepEqual(plan.deletes.map((d) => d.kind).sort(), ['relation', 'revision', 'thought']);
	assert.deepEqual(plan.puts.map((p) => p.kind), ['garden']);
	const outcome = await release.executeRelease(plan, writer(okFetch));
	assert.equal(outcome.status, 'complete');
	assert.equal(db.prepare('SELECT COUNT(*) AS n FROM published_records WHERE graph_id = ?').get(graphId).n, 4);
});

test('a failed write leaves the manifest alone and the next publish retries only the gap', async () => {
	db.prepare('INSERT INTO working_set_items (working_set_id, thought_id) VALUES (?, ?)').run(ws, b);
...
test('withdrawing the garden deletes the front door first, then everything', async () => {
	...
	assert.equal(outcome.status, 'complete');
	const deletions = calls.filter((c) => c.url.includes('deleteRecord'));
