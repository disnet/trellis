# Trellis: Design Doc (v3)

*A synthesis of `design.md` and `design-2.md`. The framing, scope discipline, and no-chat bet come from the former; the execution machinery — fixtures-first sequencing, data model, agent contract, proposal-tray spec, risks — from the latter.*
...
4. **Stable identity over stable wording.** A thought can be revised, summarized, or displayed in several views without breaking references to it.
5. **Typed links, agent-maintained.** Typed relations are what let the agent do real work — find contradictions written months apart, flag that a load-bearing claim is still tentative, notice a concept reinvented under a new name. Typed ontologies historically collapse into "related" when humans must maintain them; the bet is they survive if the human mostly just writes and the agent maintains the types.
6. **Human-visible memory.** Long-term memory appears as inspectable structure — a relevant prior claim surfaced at the edge of the workspace, not "we discussed this." The agent never holds a richer private account of your thinking than you can see.
7. **Structure without premature bureaucracy.** Freeform capture stays easy; the system proposes structure afterward. The person is never required to classify anything during capture.
8. **Views are projections.** Canvas, outline, argument map, and document are lenses over the same objects. Layout and presentation are not the knowledge model.
9. **Chat is compost — and absent from v0.** Useful conversation results get reified into the graph; the transcript survives only as provenance. v0 goes further: there is no chat surface at all (see "Key design decision" below).
...
**No Threads in v0.** `design-2.md` proposed Threads as a durable primitive while also listing "does the thread concept add value?" as an open question. Resolution: cut them. A persisted working set covers re-entry — the part of Threads worth keeping. Branch/merge of lines of inquiry is deferred until the core loop proves out.
...
Snapshots in `ThoughtRevision` are simpler and safer than event sourcing at this stage. `ProposedOperation.payload` stays validated JSON until the operation vocabulary stabilizes.
...
Use it for a real thinking project (e.g. this design itself) for 1–2 weeks, logging against the measures below. A moderated multi-participant study with a chat baseline (per `design-2.md` Phase 3) is deferred until the loop proves out on ourselves.
...
The final ontology; the long-term storage/event model; local-first vs. cloud-first; whether canvas or another projection is the mature default; autonomous agent permissions; whether accepted agent wording becomes "human-owned" or retains mixed authorship; collaboration semantics; migration from existing PKBs.
